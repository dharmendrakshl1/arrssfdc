...
@Path("/servicemastercustomerpartqueryoperation")
public class SMCustomerPartQueryOperationService {

	Logger logger = Logger.getLogger(SMCustomerPartQueryOperationService.class);
	
	@POST
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	public Response vmiOrderOperation(QueryPartnumberInput customerInput){
		logger.info("Entering - com.arris.sfdc.api.rest.MCustomerPartQueryOperationService.MCustomerPartQueryOperation(OrderImportInput) : "+customerInput);
		
		SMCustomerOperationServiceProvider sMCustomerOperationServiceProvider = new SMCustomerOperationServiceProvider();
		QueryPartnumberOutput customerOutput = null;
		try{
		customerOutput = sMCustomerOperationServiceProvider.smCustomerQueryOperation(customerInput);
		
		logger.info("Leaving - com.arris.sfdc.api.rest.MCustomerPartQueryOperationService.MCustomerPartQueryOperation(OrderImportInput) - orderImportOutput : "+customerOutput);
