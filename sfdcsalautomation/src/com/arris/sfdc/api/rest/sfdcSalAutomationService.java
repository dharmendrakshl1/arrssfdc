...
import com.arris.sfdc.service.provider.SalAutomationServiceProvider;
...
@Path("/slaautomationselect")
public class sfdcSalAutomationService {
	
	Logger logger = Logger.getLogger(sfdcSalAutomationService.class);
	
	@POST
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	public Response SalQueryAttributeType(InputElement salInputElement){
		System.out.println("the first line of the code");
		logger.info("Entering - com.arris.sfdc.api.rest.sfdcSalAutomationService.QuerySalAutomation(InputElement salInputElement) : " + salInputElement);
		Attribute1Results salOutputElement = null;
		try{
		SalAutomationServiceProvider SalAutomationServiceProvider = new SalAutomationServiceProvider();
		salOutputElement = SalAutomationServiceProvider.QuerySalAutomation(salInputElement);
		logger.info("Leaving - com.arris.sfdc.api.rest.sfdcSalAutomationService.QuerySalAutomation(salInputElement) - salOutputElement : "+salOutputElement);
		return Response.status(200).entity(salOutputElement).build();
		}catch(LoginFault e){
			logger.error("LoginFault Error : ERROR_CODE : "+e.getExceptionCode()+", ERROR_MESSAGE : "+e.getExceptionMessage());
			return Response.status(401).header("statusMessage", "ERROR_CODE : "+e.getExceptionCode()+", ERROR_MESSAGE : "+e.getExceptionMessage()).entity(salOutputElement).build();
			
		}catch(InvalidFieldFault e){
			logger.error("InvalidFieldFault Error : ERROR_CODE : "+e.getExceptionCode()+", ERROR_MESSAGE : "+e.getExceptionMessage());
			return Response.status(500).header("statusMessage", "ERROR_CODE : "+e.getExceptionCode()+", ERROR_MESSAGE : "+e.getExceptionMessage()).entity(salOutputElement).build();
			...
			return Response.status(500).header("statusMessage", "ERROR_CODE : "+e.getFaultCode()+", ERROR_MESSAGE : "+e.getMessage()).entity(salOutputElement).build();
			
		}catch(ConnectionException e){
			logger.error("ConnectionException Exception : ERROR_MESSAGE : "+e.getMessage());
			return Response.status(500).header("statusMessage", "ERROR_MESSAGE : "+e.getMessage()).entity(salOutputElement).build();
