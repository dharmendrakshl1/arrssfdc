...
import com.arris.sfdc.service.provider.OperationalSalServiceProvider_Update;
...
@Path("/operationalsalupdate")

public class sfdcOperationalSalSerivce_Update {

	Logger logger = Logger.getLogger(sfdcOperationalSalSerivce_Update.class);

	@POST
	@Consumes(MediaType.APPLICATION_XML)
	@Produces(MediaType.APPLICATION_XML)
	public Response OperationalSalQueryFlex(ApprovalLimitInput updateInputElement) {
		...
		logger.info(
				"Entering - com.arris.sfdc.api.rest.sfdcOperationalSalSerivce_QueryApproval.OperationalSalQueryFlex(InputElement approvalInputElement) : "
						+ updateInputElement);
		ApprovalLimitOutput updateOutputElement = null;
		try{
		OperationalSalServiceProvider_Update OperationalSalServiceProvider = new OperationalSalServiceProvider_Update();
		updateOutputElement = OperationalSalServiceProvider.updateCurrencyType(updateInputElement);
		logger.info(
				"Leaving - com.arris.sfdc.api.rest.sfdcOperationalSalSerivce_QueryApproval.OperationalSalQueryFlex(approvalInputElement) - approvalOutputElement : "
						+ updateOutputElement);
		return Response.status(200).entity(updateOutputElement).build();
		}
		catch(LoginFault e){
			System.out.println("LoginFault ....... : "+e.getExceptionMessage());
			return Response.status(401).entity(updateOutputElement).build();
